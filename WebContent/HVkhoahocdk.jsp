    ...
    <!--End Header section  -->
         	...
         	select khoahoc.TenKH,khoahoc.GV,khoahoc.Ngaybatdau,khoahoc.Ngayketthuc,khoahoc.LichHoc,khoahoc.IDKH
				...
				from khoahoc,dkkh
						...
						where khoahoc.IDKH=dkkh.IDKH and  dkkh.uname='<%=session.getAttribute("userid")%>'
        ...
        <thead>
                    ...
                    <c:forEach var="row" items="${result.rows}">
							<tbody>
								...
								<tr>
									<td><a href="HVhoc.jsp?id=<c:out value="${row.IDKH}"/>"><c:out value="${row.TenKH}" /></a></td>
									<td><c:out value="${row.GV}" /></td>
									<td><c:out value="${row.Ngaybatdau}" /></td>
									<td><c:out value="${row.Ngayketthuc}" /></td>
									<td><c:out value="${row.LichHoc}" /></td>
									<td><a class="btn btn-danger btn-xs" href="javascript:confirmGo('Bạn muốn xóa khóa học này','HVdakhmh.jsp?id=
									<c:out value="${row.IDKH}"/>')"><span
