...
<body>
	...
	<sql:query dataSource="${dbsource}" var="result">
            				...
            				where baigiang.IDKH=${param.IDKH }
				...
				<div class="col-sm-9">
					<div class="row">
						...
						<div class="col-md-12" style="border: 1px solid black">
							...
							<center><h3 class="tieude">Quản lý khóa học - Tài liệu</h3></center>
							...
							<table class="table table-striped">
								...
								<tr>
									...
									<th>Tuần học</th>
									<th>Tên bài giảng</th>
									<th><center>Link Video bài giảng</center></th>
									<th>Tài liệu</th>
									<th><center>Thao tác</center></th>
							...
							<c:forEach var="row" items="${result.rows}">
										<tbody>
											...
											<tr>
												<td><c:out value="${row.Tuan}" /></td>
												<td><c:out value="${row.TenBaiGiang}" /></td>
												<td><c:out value="${row.LinkVideo}" /></td>
												<td><c:out value="${row.decuong}" /></td>
												
												<td><a class="btn btn-primary btn-xs" href="suabaigiang.jsp?ID=<c:out value="${row.ID}"/>" > <span
															class="glyphicon glyphicon-pencil"></span>Sửa</a></td>
												<td><a class="btn btn-danger btn-xs" href="javascript:confirmGo('Bạn muốn xóa bài giảng này?','xoabg.jsp?id=<c:out value="${row.ID}"/>&IDKH=<c:out value="${row.IDKH}"/>')"><span
															class="glyphicon glyphicon-trash"></span>Xóa</a></td>
							...
							<table class="table table-striped">
							<div class="col-md-12">
									<form method="post" enctype="multipart/form-data" action="insertbg">
									<input type="hidden" value="${param.IDKH}" name="IDKH">
									<span><i class="glyphicon glyphicon-pencil"> </i> Tuần
										học </span> 
										
									<input type="text" name="Tuan" class="form-control"
										placeholder=" Tuần học"> 
										
									<span><i class="glyphicon glyphicon-pencil"> </i> Tên bài giảng </span> 
										
									<input type="text" name="BaiGiang" class="form-control"
										placeholder=" Tên bài giảng"> 
										
									<span><i
										class="glyphicon glyphicon-pencil"> </i> Link Video </span>
									 <input
										type="text" name="LinkVideo" class="form-control"
										placeholder="Link Video">
									 <span><i
										class="glyphicon glyphicon-pencil"> </i>Tài Liệu </span>
										<input type ="text" name = "tailieu" class="form-control" placeholder="Tên tài liệu">
									 <input
										type="file" name="link" class="form-control">
