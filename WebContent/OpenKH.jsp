...
<head>
    ...
    <!--[if IE]>
        ...
        <![endif]-->
    <title>Giảng viên - Mở khóa học</title>
  ...
  .controller( "RegisterCtrl",['$scope', function($scope) {
    ...
    <!--End Header section  -->
        ...
        <sql:update dataSource="${dbsource}" var="result">
            INSERT INTO khoahoc( TenKH, GV, Ngaybatdau , Ngayketthuc , LichHoc , DiaDiem ) VALUES (?,?,?,?,?,?);
            <sql:param value="${param.TenKhoaHoc}" /> 
            <sql:param value="${userid}" /> 
            <sql:param value="${param.NgayBatDau}" />
            <sql:param value="${param.NgayKetThuc}" />
            <sql:param value="${param.LichHoc}" />
            <sql:param value="${param.DiaDiemHoc}" />
        ...
        <h2 class="text-center" style="background:#333;color:#0adb48">THÔNG BÁO</h2>
        		 ...
        		 <c:if test="${result>=1}">
        				 <table class="table table-striped" style="background: white">
						 	...
						 	<center><a href="GVCackhoahocdamo.jsp">Quay lại trang các khóa học đã mở </a> </center>
